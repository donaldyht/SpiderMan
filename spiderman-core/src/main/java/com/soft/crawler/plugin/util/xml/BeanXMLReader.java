...
import com.soft.crawler.plugin.util.ClassUtil;
import com.soft.crawler.plugin.util.ReflectUtil;
...
public class BeanXMLReader implements XMLReader {
			...
			for (Field f : fields) {
				if (List.class.isAssignableFrom(f.getType())) {
					ParameterizedType pt = (ParameterizedType) f
							.getGenericType();
					/**
					 * 如果声明的属性类型是List集合，则获取它的泛型参数，由于List集合只能设置一个
					 * 泛型参数，所以[0]就是那个唯一的泛型参数类型
					 */
	...
	public BeanXMLReader(String content) {
	...
	private <T> Document createDoc(List<T> tList) throws DocumentException,
		...
		T t;
		Document doc = reader.read(new InputSource(new ByteArrayInputStream(this.content.getBytes("utf-8"))));
		// 列出beans下的所有bean元素节点
		String sub;
