...
import com.soft.crawler.fetcher.PageFetcher;
import com.soft.crawler.fetcher.Status;
import com.soft.crawler.xml.Site;

@SuppressWarnings("deprecation")
public class PageFetcherImpl implements PageFetcher{
	Logger logger=Logger.getLogger(PageFetcherImpl.class); 
	...
	public void init(Site _site) {
		...
		if (_site != null) {
			...
			if (this.site.getHeaders() != null && this.site.getHeaders().getHeader() != null){
				for (com.soft.crawler.xml.Header header : this.site.getHeaders().getHeader()){
					this.addHeader(header.getName(), header.getValue());
				}
			}
			if (this.site.getCookies() != null && this.site.getCookies().getCookie() != null){
				for (com.soft.crawler.xml.Cookie cookie : this.site.getCookies().getCookie()){
	...
	public FetchResult fetch(FetchRequest req) throws Exception{
		...
		try {
			...
			//执行get访问，获取服务端返回内容
			//HttpGet get1=new HttpGet("http://item.m.jd.com/product/2122207.html");
//			System.out.println("***********************************************");
//			System.out.println(get.getURI().toString());
//			System.out.println("***********************************************");;
//			HttpGet get1=new HttpGet(get.getURI().toString());
...
//			}
	...
	private Page load(HttpEntity entity) throws Exception {
		...
		content=convertUnicode(content).replaceAll("script", "").replaceAll("javascript", "");;//add by jhj 2015-10-31 13:12
